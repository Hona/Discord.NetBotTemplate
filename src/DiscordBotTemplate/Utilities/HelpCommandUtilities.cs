...
namespace DiscordBotTemplate.Utilities
{
    ...
    {
        public static Embed GetModuleHelpEmbed(ModuleInfo module, ICommandContext context, IServiceProvider services, Config config) =>
            new EmbedBuilder()
                .BuildTitle(module)
        ...
        private static EmbedBuilder AddCommandField(this EmbedBuilder embedBuilder, CommandInfo command, ModuleInfo module, Config config)
        {
        ...
        {
            if (string.IsNullOrEmpty(command.Summary)) return "No description provided";
